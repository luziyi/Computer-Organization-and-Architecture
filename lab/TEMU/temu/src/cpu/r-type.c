...
make_helper(and) {
    ...
    sprintf(assembly, "and   %s,   %s,   %s", REG_NAME(op_dest->reg),
...
make_helper(nor) {
    decode_r_type(instr);
    reg_w(op_dest->reg) = ~(op_src1->val | op_src2->val);
    sprintf(assembly, "nor   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(xor) {
    decode_r_type(instr);
    reg_w(op_dest->reg) = (op_src1->val ^ op_src2->val);
    sprintf(assembly, "xor   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(sllv) {
    ...
    sprintf(assembly, "sllv   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(sll) {
    ...
    sprintf(assembly, "sll   %s,   %s,   0x%02x", REG_NAME(op_dest->reg),
            REG_NAME(op_src2->reg), instr & SHAMT_MASK);
}

make_helper(srav) {
    decode_r_type(instr);
    reg_w(op_dest->reg) = ((int32_t)op_src2->val >> ((op_src1->val) & 0x1f));
    sprintf(assembly, "srav   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(sra) {
    decode_r_type(instr);
    reg_w(op_dest->reg) =
        ((int32_t)op_src2->val >> ((instr & SHAMT_MASK) >> 6));
    sprintf(assembly, "sra   %s,   %s,   0x%02x", REG_NAME(op_dest->reg),
            REG_NAME(op_src2->reg), instr & SHAMT_MASK);
}
...
make_helper(srlv) {
    ...
    sprintf(assembly, "srlv   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(srl) {
    decode_r_type(instr);
    reg_w(op_dest->reg) =
        (uint32_t)(op_src2->val) >> ((instr & SHAMT_MASK) >> 6);
    sprintf(assembly, "srl   %s,   %s,   0x%02x", REG_NAME(op_dest->reg),
            REG_NAME(op_src2->reg), instr & SHAMT_MASK);
}
...
make_helper(add) {
    ...
    if (((int)op_src1->val > 0 && (int)op_src2->val > 0 && temp < 0) ||
        ((int)op_src1->val < 0 && (int)op_src2->val < 0 && temp > 0)) {
        if (cpu.cp0.status.EXL ==
            0) {  // 如果此时异常级为0，即还未有异常发生（不支持嵌套异常）
            cpu.cp0.cause.ExcCode = Ov;  // 记录发生了溢出异常
            cpu.cp0.EPC =
                cpu.pc;          // 存放异常返回地址（由软件实现具体的返回地址）
    ...
    sprintf(assembly, "add   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(addu) {
    decode_r_type(instr);
    reg_w(op_dest->reg) = op_src1->val + op_src2->val;
    sprintf(assembly, "addu   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(sub) {
    ...
    sprintf(assembly, "sub   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(subu) {
    decode_r_type(instr);
    reg_w(op_dest->reg) = op_src1->val - op_src2->val;
    sprintf(assembly, "subu   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(slt) {
    ...
    sprintf(assembly, "slt   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(sltu) {
    ...
    sprintf(assembly, "sltu   %s,   %s,   %s", REG_NAME(op_dest->reg),
            REG_NAME(op_src1->reg), REG_NAME(op_src2->reg));
}
...
make_helper(div) {  // 什么div zero？
    ...
    cpu.lo = quotient;
    cpu.hi = remainder;
    sprintf(assembly, "div   %s,   %s", REG_NAME(op_src1->reg),
...
make_helper(divu) {
    ...
    cpu.lo = quotient;
    cpu.hi = remainder;
    sprintf(assembly, "divu   %s,   %s", REG_NAME(op_src1->reg),
            REG_NAME(op_src2->reg));
}
...
make_helper(mult) {
    ...
    cpu.hi = hi;
    sprintf(assembly, "mult   %s,   %s", REG_NAME(op_src1->reg),
            REG_NAME(op_src2->reg));
}
...
make_helper(multu) {
    ...
    cpu.lo = lo;
    cpu.hi = hi;
    sprintf(assembly, "multu   %s,   %s", REG_NAME(op_src1->reg),
            REG_NAME(op_src2->reg));
}
